...
/*-------------------------------------------------*/
PROCESS(iotWaterNode, "Main - IoT WaterNode");
AUTOSTART_PROCESSES(&iotWaterNode);
/*-------------------------------------------------*/
...
static void publishData(void)
{
    ...
    phVRaw = adc_zoul.value(ZOUL_SENSORS_ADC3) * 1.75;
    ph = 7000000 - ((21500 - phVRaw) * 242);

    if (dht22_read_all(&temp, &hum) != DHT22_ERROR)
    {
        printf("[PUB DATA] - PH = %d.%u \n", ph / 1000000, ph % 1000000);
        printf("[PUB DATA] - TEMP = %d.%u C\n", temp / 10, temp % 10);

        sprintf(payload,
                "{\"node_type\":\"%s\",\"node_name\":\"%s\", \"data\": {\"temp\": %d.%u, \"ph\": %d.%u}}",
                nodeType, nodeName, temp / 10, temp % 10, ph / 1000000, (ph % 1000000) % 100);
...
PROCESS_THREAD(iotWaterNode, ev, data)
{
    PROCESS_BEGIN();
    ...
    printf("[INFO] - Starting IoT WaterNode\n");
    ...
    while (1)
    {
        ...
        else if (state == STATE_DISCONNECTED)
        {
            ...
            mqtt_register(&conn, &iotWaterNode, clientID, mqttEventCallback, MAX_TCP_SEGMENT_SIZE);
